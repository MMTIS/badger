...
from utils.refs import getRef, getFakeRefByClass
from transformers.references import replace_with_reference_inplace
from utils.utils import get_object_name, get_element_name_with_ns
...
def load_embedded(db: Database, clazz: T, filter, cursor=False):
    ...
    with db.env.begin(db=db.env.open_db(b"_embedding"), buffers=True, write=False) as txn:
        ...
        for key, value in cursor:
            ...
            parent_class, parent_id, parent_version, embedding_class, embedding_id, embedding_version, *_ = pickle.loads(
                value)
...
def load_local(db: Database, clazz: T, limit=None, filter=None, cursor=False, embedding=True, embedded_parent=False,
               cache=True) -> list[T]:
    ...
    with db.env.begin(db=db.db_referencing, buffers=True, write=False) as src3_txn:
        ...
        for _key, value in cursor:
            ...
            if ref_class not in list_classes:
                results = load_local(db, db.get_class_by_name(ref_class), limit=1, filter=ref_id, cursor=True,
                                     embedding=True, embedded_parent=True)
                if len(results) > 0:
                        ...
                        with db.env.begin(db=db.db_embedding, buffers=True, write=False) as src_txn2:
                            ...
                            if cursor2.set_range(prefix):  # Position cursor at the first key >= prefix
                                for key2, value2 in cursor2:
                                    ...
                                    embedding_class, embedding_id, embedding_version, embedding_path = cloudpickle.loads(
                                        value2)
                                    if (embedding_class, db.serializer.encode_key(embedding_id, embedding_version,
                                                                                  db.get_class_by_name(
                                                                                          embedding_class))) in existing_ids:
                                    ...
                                    if cursor2.set_range(prefix):  # Position cursor at the first key >= prefix
                                        for key2, value2 in cursor2:
                                            ...
                                            embedding_class, embedding_id, embedding_version, embedding_path = cloudpickle.loads(
                                                value2)
                                            if (embedding_class,
                                                db.serializer.encode_key(embedding_id, embedding_version,
                                                                         db.get_class_by_name(
                                                                                 embedding_class))) in existing_ids:
