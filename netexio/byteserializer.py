...
class Database:
    ...
    def resolve(self) -> None:
        ...
        with (self.env.begin(write=True) as txn):
            db_unresolved = self.env.open_db(DB_UNRESOLVED)
            db_id_idx = self.env.open_db(DB_ID_IDX)
            db_reference_forward = self.env.open_db(DB_REFERENCE_FORWARD)
            db_reference_inward = self.env.open_db(DB_REFERENCE_INWARD)
