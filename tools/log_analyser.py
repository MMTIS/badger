...
LOG_PATTERN = re.compile(r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) - script_runner - (\w+) - (.+)$")

# Pattern to match step start entries
STEP_START_PATTERN = re.compile(r"^(.+?) - step: (\d+): (.+)$")

# Pattern to match execution time entries
EXECUTION_TIME_PATTERN = re.compile(r"^Execution time: ([\d.]+) seconds for (.+? - step: \d+: )?(?:(.+) \(?([\d.]+)s\)?|(.+))$")

# Better pattern for execution time
EXEC_TIME_PATTERN = re.compile(r"^Execution time: ([\d.]+) seconds for (.+)$")
...
BLOCK_STEP_PATTERN = re.compile(r"^([a-zA-Z0-9_-]+) - step: (\d+): (.+)$")

# Pattern for error messages that indicate script termination
SCRIPT_ERROR_PATTERN = re.compile(r"^Script (\S+) returned an error\. Terminating the block of scripts: (\S+)$")
...
class LogEntry:
    ...
    def is_within_days(self, days: int, reference_date: datetime | None = None) -> bool:
...
class ErrorEntry:
    ...
    def __init__(self, error_message: str, stacktrace: str, script_name: str = "", block_name: str = "", execution_time: float = 0.0):
    ...
    while i < len(error_line_numbers):
        ...
        if script_error_match:
            ...
            if prev_entry is not None:
                ...
                error = ErrorEntry(
                    error_message=error_message, stacktrace=full_traceback, script_name=script_name, block_name=block_name, execution_time=execution_time
                )
                errors.append(error)
            ...
            error = ErrorEntry(error_message=error_message, stacktrace="", script_name=script_name, block_name=block_name, execution_time=0.0)
            errors.append(error)
...
def generate_markdown_report(sequences: List[RunSequence], errors: List[ErrorEntry], file_path: str = "") -> str:
...
def main():
    parser = argparse.ArgumentParser(description='Analyse script_runner.py log files and generate a markdown summary.')
    parser.add_argument('-i', '--input', type=str, required=True, help='Log file or folder containing .log files to analyse')
    parser.add_argument('-t', '--days', type=int, default=10, help='Number of days back to consider (default: 10)')
