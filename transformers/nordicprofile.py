...
class NordicProfile:
    ...
    def projectNetworkFromLines(self, lines: List[Line]) -> Network:
        ...
        network: Network = Network(
            id=getId(Network, self.codespace, "1"), version=lines[0].version, groups_of_lines=GroupsOfLinesInFrameRelStructure(group_of_lines=[group_of_lines])
        )
        return network
    ...
    @staticmethod
    def projectDayTypeAssignmentToDayTypeAssignmentDate(
        day_type_assignment: DayTypeAssignment, operating_periods: Dict[str, UicOperatingPeriod]
    ) -> List[DayTypeAssignment]:
        ...
        if isinstance(day_type_assignment.uic_operating_period_ref_or_operating_period_ref_or_operating_day_ref_or_date, OperatingPeriodRef):
            ...
            for i in range(0, len(operational_dates)):
                day_type_assignment = DayTypeAssignment(
                    id=day_type_assignment.id,
                    version=day_type_assignment.version,
    ...
    def getServiceFrameShared(
        ...
        service_frame = ServiceFrame(
            id=getId(ServiceFrame, self.codespace, "1"),
    ...
    def getServiceCalendarFrameShared(self, day_types: List[DayType], day_type_assignments: List[DayTypeAssignment]):
        ...
        service_calendar_frame = ServiceCalendarFrame(
            id=getId(ServiceCalendarFrame, self.codespace, "1"), version=self.version.version, day_type_assignments=day_type_assignments, day_types=day_types
        )
    ...
    def getResourceFrameShared(self, operators: List[Operator]) -> ResourceFrame:
        ...
        resource_frame = ResourceFrame(
            id=getId(ResourceFrame, self.codespace, "1"),
            version=self.version.version,
    ...
    def getShared(
        self, resource_frame: List[ResourceFrame], service_frame: List[ServiceFrame], service_calendar_frame: List[ServiceCalendarFrame]
    ) -> PublicationDelivery:
        return PublicationDelivery(
            ...
            data_objects=DataObjectsRelStructure(choice=resource_frame + service_frame + service_calendar_frame),
        )
    ...
    def getTimetableFrame(self, line: Line, service_journeys: List[ServiceJourney], journey_patterns: List[object]) -> TimetableFrame:
        ...
        return TimetableFrame(
            id=line.id.replace("Line", "TimetableFrame"),
            version=self.version.version,
    ...
    def getServiceFrame(self, line: Line, journey_patterns: List[JourneyPattern], routes: List[Route]) -> ServiceFrame:
        ...
        return ServiceFrame(
            id=line.id.replace("Line", "ServiceFrame"),
            version=self.version.version,
    ...
    def getLineDelivery(
        self, line: Line, resource_frame: [ResourceFrame], service_frame: [ServiceFrame], timetable_frame: [TimetableFrame]
    ) -> PublicationDelivery:
        return PublicationDelivery(
            version="1.08:NO-NeTEx-networktimetable:1.3",
            publication_timestamp=XmlDateTime.now(),
